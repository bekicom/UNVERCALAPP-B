...
router.get("/stores", authMiddleware, async (req, res) => {
  const stores = await Store.find(tenantFilter(req)).sort({ createdAt: -1 }).lean();
  res.json({ stores });
});

// Create store
router.post("/stores", authMiddleware, requireAdmin, async (req, res) => {
  const { name, storeCode, address, phone, note } = req.body;
  if (!name || !storeCode) {
    return res.status(400).json({ message: "Nomi va do'kon kodi majburiy" });
  }

  const exists = await Store.exists(tenantFilter(req, { storeCode }));
  if (exists) {
    return res.status(409).json({ message: "Bu do'kon kodi allaqachon mavjud" });
  }

  const store = await Store.create(withTenant(req, { name, storeCode, address, phone, note }));
  res.json({ store });
});
